...
type BadgeStatus = "success" | "warning" | "danger" | "info" | "neutral" | "processing";
...
const statusConfig: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  credit:     { bg: "bg-emerald-500/10", text: "text-emerald-600", dot: "bg-emerald-500" },
  ...
  info:       { bg: "bg-ocean-500/10",   text: "text-ocean-600",   dot: "bg-ocean-500" },
  processing: { bg: "bg-ocean-500/10",   text: "text-ocean-600",   dot: "bg-ocean-500" },
  neutral:    { bg: "bg-brand-500/5",    text: "text-brand-500/60",dot: "bg-brand-500/40" },
};
...
const StatusBadge: React.FC<StatusBadgeProps> = ({
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full font-black ${config.bg} ${config.text} ${sizeStyles} ${className}`}>
